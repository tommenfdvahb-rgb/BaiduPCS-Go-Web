...
Web 页面支持使用百度网盘 Cookie 登录或切换账号，并分为“文件概览”“文件上传”和“文件下载”三个标签页：概览页用于浏览目录、新建文件夹和重命名，上传页用于选择目录、上传文件并查看当前上传任务，下载页用于加入下载队列、查看下载进度和下载历史；文件列表按页展示。默认只监听本机地址；如果使用 `--addr 0.0.0.0:8080` 对外提供服务，请自行增加访问控制。
