...
Web 页面支持使用百度网盘 Cookie 登录或切换账号，并分为“文件概览”“文件上传”和“文件下载”三个标签页：概览页用于浏览目录、新建文件夹、重命名并直接接收文件，上传页支持浏览器文件上传和指定服务器本地文件/目录路径上传，并查看当前任务和上传历史，下载页可选择浏览器接收或加入服务器下载队列并查看下载进度和下载历史；文件列表按页展示。默认只监听本机地址；如果使用 `--addr 0.0.0.0:8080` 对外提供服务，请自行增加访问控制。
